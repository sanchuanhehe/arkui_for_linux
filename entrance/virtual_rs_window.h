...
// Linux/Wayland port: WindowView is a plain C++ class (see WindowView.h). The
// macOS NSViewController has no Wayland analogue; the (unused) member that held
// it is kept as an opaque pointer for source parity.
class WindowView;
...
private:
    ...
    std::shared_ptr<void> viewController_ = nullptr;
