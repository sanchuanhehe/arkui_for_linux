...
// Linux/Wayland native port: ported from adapter/macos/osal/dynamic_module_helper.mm.
// The only macOS-specific bit was BuildDylibPath (NSBundle pathForResource +
// .dylib); on Linux dynamic component libs are plain libarkui_<name>.so next to
// the executable. The dlopen/dlsym cache logic is identical.
...
// Static module instances for components bundled into libace_static_linux
...
void InitializeStaticModules()
{
    ...
    std::call_once(initFlag, []() {
        ...
        g_menuItemGroupModule.reset(
            reinterpret_cast<DynamicModule*>(OHOS_ACE_DynamicModule_Create_MenuItemGroup()));
        LOGI("InitializeStaticModules finished");
    });
}

const std::unordered_map<std::string, std::string> soMap = {
    { "Marquee", "marquee" }, { "Stepper", "stepper" }, { "StepperItem", "stepper" },
    { "Slider", "slider" }, { "Checkbox", "checkbox" }, { "CheckboxGroup", "checkbox" },
    { "Gauge", "gauge" }, { "Rating", "rating" }, { "FlowItem", "waterflow" },
    { "WaterFlow", "waterflow" }, { "Counter", "counter" }, { "Sidebar", "sidebar" },
    { "ColumnSplit", "linearsplit" }, { "RowSplit", "linearsplit" }, { "Radio", "radio" },
    { "QRCode", "qrcode" }, { "TimePicker", "timepicker" }, { "TimePickerDialog", "timepicker" },
    { "Indexer", "indexer" }, { "Hyperlink", "hyperlink" }, { "PatternLock", "patternlock" },
    { "CalendarPicker", "calendarpicker" }, { "CalendarPickerDialog", "calendarpicker" },
    { "SymbolGlyph", "symbol" }, { "DataPanel", "datapanel" }, { "Richeditor", "richeditor" },
    { "Search", "search" }, { "TextClock", "textclock" }, { "DynamicLayout", "dynamiclayout" },
};
...
// Linux: libarkui_<name>.so next to the executable.
std::string BuildSoPath(const std::string& moduleName)
{
    return GetExecutableDir() + "/" + DYNAMIC_MODULE_LIB_PREFIX + moduleName + DYNAMIC_MODULE_LIB_POSTFIX;
}

std::pair<DynamicModule*, void*> LoadDynamicLibrary(const std::string& name, const std::string& moduleName)
{
    std::string soPath = BuildSoPath(moduleName);
...
DynamicModule* InsertModuleToCache(const std::string& name, DynamicModule* module, void* handle, std::mutex& mutex,
    std::unordered_map<std::string, std::unique_ptr<DynamicModule>>& moduleMap)
{
    std::lock_guard<std::mutex> lock(mutex);
    auto iter = moduleMap.find(name);
