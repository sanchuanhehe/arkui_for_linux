...
// Linux/Wayland port of the macOS Window. nullptr is a language keyword; uint32_t
// comes from <cstdint>. No AppKit/Carbon includes (and thus no Component macro
// hack) are needed on Linux; the NSWindow/NSView surface is replaced by the
// Wayland WindowView (WindowView.h).
#include <atomic>
#include <cstdint>
#include <limits>
#include <memory>
...
#include "stage/uicontent/ui_content_impl.h"
...
// Linux/Wayland port: monotonic window-id source (was [InstanceIdGenerator getAndIncrement]).
std::atomic<uint32_t> Window::tempWindowId { 1 };
...
// Linux/Wayland port: the macOS NSColor <-> ARGB helpers were unused on this
// path and are dropped (no AppKit color type on Linux).
    ...
    std::shared_ptr<OHOS::AbilityRuntime::Platform::Context> context, void* windowView)
{
    ...
    uint32_t windowId = tempWindowId.fetch_add(1);
    ...
    std::shared_ptr<OHOS::Rosen::WindowOption> option)
{
    ...
    auto window = std::shared_ptr<Window>(new Window(context, windowId), DummyWindowRelease);
    // Linux/Wayland port: the AppKit/UIKit WindowView becomes a plain C++ WindowView.
...
{
    // Stage C: the macOS path queried StageApplication's top NSViewController for
    // its instance name and resolved the WindowView via WindowViewAdapter. The
    // Wayland stage host (StageApplication analogue) is not wired yet, so resolve
    // through whichever WindowView the adapter currently tracks, if any.
    (void)context;
    WindowView* windowView = static_cast<WindowView*>(
        OHOS::AbilityRuntime::Platform::WindowViewAdapter::GetInstance()->GetWindowView(""));
...
WMError Window::ShowWindow()
{
    ...
    // Linux/Wayland port: there is no parent NSView to attach to; a Wayland
    // toplevel maps itself when its surface is committed. ShowOnView builds/commits
    // the wl_surface chain.
    if (windowView_->ShowOnView(nullptr)) {
...
WMError Window::MoveWindowTo(int32_t x, int32_t y)
{
    ...
    // Stage C: Wayland clients cannot position their own toplevels (the compositor
    // owns placement). Record the requested rect for queries; the move is a no-op.
    rect_.posX_ = x;
    rect_.posY_ = y;
    return WMError::WM_OK;
}
...
WMError Window::SetOnTop(bool status)
{
    ...
    if (status) {
        windowView_->zOrder = std::numeric_limits<int64_t>::max();
    } else {
...
WMError Window::SetTouchHotAreas(const std::vector<Rect>& rects)
{
    ...
    int size = rects.size();
    if (size == 0) {
        windowView_->SetTouchHotAreas(nullptr, 0);
        return WMError::WM_OK;
    }
    // Linux/Wayland port: CGRect[] -> WindowViewRect[]. Density (was NSScreen
    // backingScaleFactor) maps physical px to logical coordinates.
    float scale = density_ > 0 ? density_ : 1.0f;
    std::vector<WindowViewRect> hotRects(size);
    for (int i = 0; i < size; i++) {
        hotRects[i] = WindowViewRect { rects[i].posX_ / scale, rects[i].posY_ / scale,
            rects[i].width_ / scale, rects[i].height_ / scale };
    }
    windowView_->SetTouchHotAreas(hotRects.data(), size);
    return WMError::WM_OK;
}
...
WMError Window::ResizeWindowTo(int32_t width, int32_t height) {
    ...
    // Linux/Wayland port: resize the wl_egl_window (was the NSView.frame size).
    float scale = density_ > 0 ? density_ : 1.0f;
    windowView_->Resize(static_cast<int32_t>(width / scale), static_cast<int32_t>(height / scale));
...
void Window::SetWindowView(WindowView* windowView)
{
    ...
    // Linux/Wayland port: plain ownership transfer (was ObjC retain/release).
...
WMError Window::SetWindowPrivacyMode(bool isPrivacyMode) {
    ...
    // Stage C: privacy mode mapped onto the StageViewController on macOS. The
    // Wayland stage host does not expose an equivalent yet; record-and-return.
    (void)isPrivacyMode;
    return WMError::WM_OK;
}
